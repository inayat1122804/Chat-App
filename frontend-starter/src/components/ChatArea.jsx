...
const ChatArea = ({ selectedGroup, socket }) => {
  ...
  const currentUser = JSON.parse(sessionStorage.getItem("userInfo") || "null");
  useEffect(() => {
            ...
            >
              <InputGroup size="lg">
                <Input
                  ...
                  value={newMessage} // bind input value to state so typing updates newMessage
                  onChange={(e) => setNewMessage(e.target.value)} // update state on each keystroke
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
